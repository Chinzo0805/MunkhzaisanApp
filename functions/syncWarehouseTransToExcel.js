...
/**
 * Sync warehouse transactions from Firestore to Excel
 */
exports.syncWarehouseTransToExcel = functions
  .region("asia-east2")
  .https.onRequest(async (req, res) => {
    ...
    try {
      console.log("Starting sync of warehouse transactions to Excel...");
      ...
      const transactionsSnapshot = await db.collection("warehouseTransactions").get();

      if (transactionsSnapshot.empty) {
        return res.status(200).json({
          success: true,
          message: "No warehouse transactions to sync",
        });
      }
      ...
      // Prepare warehouse transaction data for Excel
      ...
      transactionsSnapshot.forEach((doc) => {
        ...
        transactionData.push([
          ...
          trans.quantity || 0,
          trans.leftover || 0,
          trans.requestedEmpID || "",
          trans.requestedEmpName || "",
          trans.projectID || "",
          trans.ProjectName || "",
        ]);
      });

      console.log(`Prepared ${transactionData.length} warehouse transactions for Excel`);
      ...
      for (let i = 0; i < transactionData.length; i += batchSize) {
        ...
        console.log(`Added batch ${Math.floor(i / batchSize) + 1} of ${Math.ceil(transactionData.length / batchSize)}`);
      }

      console.log("Warehouse transactions sync to Excel completed successfully");

      return res.status(200).json({
        success: true,
        message: "Warehouse transactions synced to Excel successfully",
        syncedCount: transactionData.length,
      });
    } catch (error) {

...
function checkPassword() {
  if (passwordInput.value === HARDCODED_PASSWORD) {
    ...
    sessionStorage.setItem('publicBountyAuth', 'true');
    errorMessage.value = '';
    loadReport();
  } else {
...
function logout() {
  ...
  sessionStorage.removeItem('publicBountyAuth');
  passwordInput.value = '';
  projects.value = [];
}
...
function getDateRange() {
  const day = selectedRange.value;
  return { from: `${selectedMonth.value}-${day}`, to: `${selectedMonth.value}-${day}` };
}
...
function exportToExcel() {
  ...
  XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet([summaryHeaders, ...summaryRows]), 'Нэгтгэл');
  for (const proj of projects.value) {
    const headers = ['Ажилтан', 'Инженер цаг', 'Инженер урамшуулал', 'Техникч цаг', 'Техникч урамшуулал', 'Илүү цаг', 'Илүү цагийн урамшуулал', 'Нийт'];
    const rows = (proj._employees || []).map(e => [e.name, e.engineerHours, e.engineerBounty, e.nonEngineerHours, e.nonEngineerBounty, e.overtimeHours, e.overtimeBounty, e.totalBounty]);
    ...
    XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet([headers, ...rows]), `#${proj.id}`.slice(0, 31));
  }
...
onMounted(() => {
  if (sessionStorage.getItem('publicBountyAuth') === 'true') {
    isAuthenticated.value = true;
    loadReport();
  }
});
